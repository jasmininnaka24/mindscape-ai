  ...
  if (userIndex !== -1) {
    ...
    if (rooms[room].length === 0) {
      ...
      reviewUserTurnSer[room] =
        (reviewUserTurnSer[room] - 1 + rooms[room].length) % rooms[room].length;
    }
    ...
    if (rooms[room].length === 1) {
      ...
      io.to(room).emit(
        ...
        isStartStudyButtonStartedList[room]
      );
    }
...
const handleUserDisconnectAssessment = (assessmentRoom, socketId) => {
  const userIndex = assessmentRoomList[assessmentRoom].findIndex(
  ...
  if (userIndex !== -1) {
    ...
    if (assessmentRoomList[assessmentRoom].length === 1) {
      ...
      io.to(assessmentRoom).emit(
        "assessment_started",
        isAssessmentButtonStartedList[assessmentRoom]
      );
      ...
      io.to(assessmentRoom).emit(
        "username_of_who_submitted",
        usernameOfWhoSubmittedPerson[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "assessment_score",
        assessmentScores[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "isSubmitted_assess",
        isSubmittedAssess[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "isAssessment_done",
        isAssessmentDoneList[assessmentRoom]
      );

      io.to(assessmentRoom).emit(
        "show_submitted_answer_modal",
        showSubmittedAnswerModalList[assessmentRoom]
      );
      io.to(assessmentRoom).emit("show_texts", showTextsList[assessmentRoom]);
      io.to(assessmentRoom).emit(
        "show_analysis",
        showAnalysisList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "show_assessment",
        showAssessmentList[assessmentRoom]
      );

      io.to(assessmentRoom).emit(
        "over_all_items",
        overAllItemsList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "pre_assessment_score",
        preAssessmentScoreList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "assessment_score_latest",
        assessmentScoreLatestList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "assessment_imp",
        assessmentImpList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "assessment_score_perf",
        assessmentScorePerfList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "completion_time",
        completionTimeList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "confidence_level",
        confidenceLevelList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "over_all_performance",
        overAllPerformanceList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "assessment_count_more_than_one",
        assessmentCountMoreThanOneList[assessmentRoom]
      );
      io.to(assessmentRoom).emit(
        "generated_analysis",
        generatedAnalysisList[assessmentRoom]
    ...
    io.to(assessmentRoom).emit(
      "assessment_user_list",
      assessmentRoomList[assessmentRoom]
    );
  }
};
...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", (data) => {
    ...
    if (userExists) {
      ...
      const duplicateUserIndex = rooms[room].findIndex(
  ...
  socket.on(
    "submitted_answer",
    ({ room, currentFailCount, submittedAnswerVal }) => {
  ...
  socket.on("join_assessment_room", (data) => {
    ...
    if (isSubmittedButtonClicked[room] === false) {
      ...
      io.to(room).emit(
        "submitted_button_response",
        isSubmittedButtonClicked[room]
      );
    } else {
      io.to(room).emit(
        "submitted_button_response",
    ...
    io.to(room).emit(
      "show_submitted_answer_modal",
      showSubmittedAnswerModalList[room]
    );
    ...
    io.to(room).emit(
      "assessment_score_latest",
      assessmentScoreLatestList[room]
    );
    ...
    io.to(room).emit(
      "assessment_count_more_than_one",
      assessmentCountMoreThanOneList[room]
    );
    io.to(room).emit("generated_analysis", generatedAnalysisList[room]);
    io.to(room).emit(
      "shuffled_choices_assessment",
      shuffledChoicesAssessmentData[room]
    );
    io.to(room).emit(
      "extracted_QA_assessment",
      extractedQAAssessmentList[room]
    );
    io.to(room).emit(
      "assessment_users_choices",
      assessmentUsersChoicesList[room]
    );
  ...
  socket.on("updated_assessment_started", ({ isStarted, assessementRoom }) => {
    ...
    io.to(assessementRoom).emit(
      "assessment_started",
      isAssessmentButtonStartedList[assessementRoom]
    );
  });
  ...
  socket.on("updated_assessment_users_choices", (data) => {
    ...
    io.to(assessementRoom).emit(
      "assessment_users_choices",
      assessmentUsersChoicesList[assessementRoom]
    );
  ...
  socket.on("updated_answers", (data) => {
    ...
    io.to(assessementRoom).emit(
      "selected_assessment_answers",
      ioSelectedAssessmentAnswers[assessementRoom]
    );
  });
  ...
  socket.on("join_discussion_room", (data) => {
    ...
    if (!discussionCreatedRoomsList[room]) {
      ...
      io.to(room).emit(
        "discussion_rooms_list",
        discussionCreatedRoomsList[room]
      );
    }

    // Check if the user is already in the room
    const existingUser = discussionRoomList[room].find(
  ...
  socket.on("created_discussion_forum", (data) => {
    ...
    discussionCreatedRoomsList[discussionForumRoom].push({
      socketId: socket.id,
      room,
      roomName,
      roomCategory,
      roomDescription,
      users: [{ socketId: socket.id, userId, username }],
      messages: [],
    });

    // Emit the updated user list and current turn to all clients in the room
    io.to(discussionForumRoom).emit(
      "discussion_rooms_list",
      discussionCreatedRoomsList[discussionForumRoom]
    );
  ...
  socket.on("join_created_room", (data) => {
    const { room, username, userId, discussionForumRoom, users, index } = data;

    socket.join(room);

    // Update the users array
    discussionCreatedRoomsList[discussionForumRoom][index].users.push({
      socketId: socket.id,
      userId,
      username,
    });
  ...
  socket.on("disconnect", () => {
    ...
    Object.keys(discussionRoomList).forEach((discussionRoom) => {
      ...
      if (Array.isArray(discussionRoomList[discussionRoom])) {
        const userIndex = discussionRoomList[discussionRoom].findIndex(
          (user) => user.socketId === socket.id
        );
        ...
        if (userIndex !== -1) {
          ...
          io.to(discussionRoom).emit(
            "discussion_room_list",
            discussionRoomList[discussionRoom]
          );
        }
      }
    });
      ...
      if (Array.isArray(discussionCreatedRoomsList[disRoom])) {
        ...
        Object.keys(discussionCreatedRoomsList[disRoom]).forEach((roomName) => {
          ...
          if (discussionCreatedRoomsList[disRoom][roomName].users) {
            const userIndex = discussionCreatedRoomsList[disRoom][
              roomName
            ].users.findIndex((user) => user.socketId === socket.id);

            if (userIndex !== -1) {
              discussionCreatedRoomsList[disRoom][roomName].users.splice(
                userIndex,
                1
              );

              if (
                discussionCreatedRoomsList[disRoom][roomName].users.length === 0
              ) {
                // Remove the room if there are no users in it
                delete discussionCreatedRoomsList[disRoom][roomName];

                // Filter out rooms with no users dynamically
                const roomsWithUsers = Object.keys(
                  discussionCreatedRoomsList[disRoom]
                ).filter(
                  (roomName) =>
                    discussionCreatedRoomsList[disRoom][roomName]?.users &&
                    discussionCreatedRoomsList[disRoom][roomName]?.users
                      .length > 0
                );
...
const RenderTrig = require("./routes/RenderTrig");
