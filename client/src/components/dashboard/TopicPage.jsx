              ...
              <tbody>
                {studyMaterials.map((item, index) => {
                  return <tr className='border-bottom-thin-gray rounded-[5px]' key={index}>
                    ...
                    <td className='text-center py-3 mcolor-800'>{item.numOfTakes}</td>
                    <td className='text-center py-3 mcolor-800'>{new Date(item.updatedAt).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })}</td>

                    <td className='text-center py-3 mcolor-800'>{item.preAssessmentScore}/{item.overAllItems}</td>
                    <td className='text-center py-3 mcolor-800'>{item.assessmentScore}/{item.overAllItems}</td>


                    <td className='text-center py-3 mcolor-800'>{item.completionTime}</td>

                    <td className='text-center py-3 mcolor-800'>{item.assessmentImp}%</td>
            ...
            <div className={`absolute top-0 modal-bg left-0 w-full h-full`}>
              <div className='flex items-center justify-center h-full'>
                <div className='relative mbg-100 min-h-[40vh] w-1/2 z-10 relative p-10 rounded-[5px]'>
                  ...
                  {showFirstText === true && (
                    <div>
                      <p className='text-center font-medium mcolor-800 pt-10'>No analysis has been recorded. Would you like to generate an analysis for the given data?</p>
                  ...
                  {showSecondText === true && (
                    <div>
                      ...
                      <p className='text-center font-medium mcolor-800 mt-5'>Kindly be advised that the data analysis process by the system AI may require 2-3 minutes, depending on your internet speed. Would you be comfortable waiting for that duration?</p>
