...
export const AssessmentPage = (props) => {
  ...
  const {
    ...
    userId,
    userListAssessment,
    setUserListAssessment,
    selectedAssessmentAnswer,
    setSelectedAssessmentAnswer,
    assessementRoom,
    isRunning,
    setIsRunning,
    seconds,
    ...
    setAssessmentCountMoreThanOne,
    generatedAnalysis,
    setGeneratedAnalysis,
    shuffledChoicesAssessment,
    extractedQAAssessment,
    setAssessmentUsersChoices,
    message,
    setMessage,
    messageList,
    setMessageList,
    isStartAssessmentButtonStarted,
    setIsStartAssessmentButtonStarted,
    setShowPreJoin,
    setIsJoined,
    setShowAssessmentPage,
    inviteMembers,
    successfullyInvited,
  ...
  const [showScoreForPreAssessment, setShowScoreForPreAssessment] =
    useState(false);
  ...
  const fetchData = async () => {
    const materialTitleResponse = await axios.get(
      `${SERVER_URL}/studyMaterial/get-material/${materialId}`
    );
    setMaterialTitle(materialTitleResponse.data.title);

    const materialCategoryResponse = await axios.get(
      `${SERVER_URL}/studyMaterialCategory/get-categoryy/${materialTitleResponse.data.StudyMaterialsCategoryId}`
    );
    setMaterialCategory(materialCategoryResponse.data.category);

    const previousSavedData = await axios.get(
      `${SERVER_URL}/DashForPersonalAndGroup/get-latest-assessment-group/${materialId}/${groupId}`
    );
    ...
    if (
      ...
      fetchedData[0].assessmentScore !== "none"
    ) {
    ...
    if (
      ...
      fetchedData[0].assessmentScore !== undefined &&
      fetchedData[0].assessmentScore === "none"
    ) {
  ...
  const updateStudyPerformance = async (overallperf) => {
    try {
      const updatedStudyPerformance = await axios.put(
        `${SERVER_URL}/studyMaterial/update-study-performance/${materialId}`,
        {
      ...
      const categoryId =
        updatedStudyPerformance.data &&
        updatedStudyPerformance.data.StudyMaterialsCategoryId;

      if (categoryId) {
        setCategoryID(categoryId);

        const extractedStudyMaterials = await axios.get(
          `${SERVER_URL}/studyMaterial/all-study-material/${categoryId}`
        );
        ...
        let calcStudyPerfVal = extractedStudyMaterialsResponse.reduce(
          (sum, item) => sum + item.studyPerformance,
          0
        );
        let overAllCalcVal = (calcStudyPerfVal / materialsLength).toFixed(2);

        await axios.put(
          `${SERVER_URL}/studyMaterialCategory/update-study-performance/${categoryId}`,
          {
  ...
  const dataForSubmittingAnswers = async (userId) => {
    ...
    if (idOfWhoSubmitted === userId) {
      ...
      if (fetchedData.length === 0) {
        let confidence = (
          (Math.round(extractedQAAssessment.length / 2) / 8) *
          100
        ).toFixed(2);
        ...
        let data = {
          ...
          assessmentScorePerf: (
            (score / extractedQAAssessment.length) *
            100
          ).toFixed(2),
          completionTime: completionTimeCalc,
          confidenceLevel:
            completionTimeCalc >= Math.round(extractedQAAssessment.length / 2)
              ? confidence
              : 100,
        ...
        const newlyFetchedDashboardData = await axios.post(
          `${SERVER_URL}/DashForPersonalAndGroup/`,
          data
        );
        ...
        if (fetchedData[0].assessmentScore === "none") {
          const improvement = Math.max(
            0,
            (
              ((score - fetchedData[0].preAssessmentScore) /
                Math.max(
                  extractedQAAssessment.length -
                    fetchedData[0].preAssessmentScore,
                  1
                )) *
              100
            ).toFixed(2)
          ...
          let confidence = (
            (Math.round(extractedQAAssessment.length / 2) /
              completionTimeCalc) *
            100
          ).toFixed(2);
          ...
          let data = {
            ...
            assessmentImp:
              parseInt(score) === parseInt(fetchedData[0].preAssessmentScore)
                ? 100
                : improvement,
            assessmentScorePerf: (
              (score / extractedQAAssessment.length) *
          ...
          const newlyFetchedDashboardData = await axios.put(
            `${SERVER_URL}/DashForPersonalAndGroup/update-data/${fetchedData[0].id}`,
            data
          );
          const newlyFetchedDashboardDataValues =
            newlyFetchedDashboardData.data;

          setAnalysisId(newlyFetchedDashboardDataValues.id);

          setOverAllItems(newlyFetchedDashboardDataValues.overAllItems);
          setPreAssessmentScore(
            newlyFetchedDashboardDataValues.preAssessmentScore
          );
          ...
          setAssessmentScorePerf(
            newlyFetchedDashboardDataValues.assessmentScorePerf
          );
          ...
          setOverAllPerformance(
            (parseFloat(newlyFetchedDashboardDataValues.assessmentImp) +
              parseFloat(newlyFetchedDashboardDataValues.assessmentScorePerf) +
              parseFloat(newlyFetchedDashboardDataValues.confidenceLevel)) /
              3
          );
        ...
        // for more than one assesssment functionality
          ...
          const improvement = Math.max(
            0,
            (
              ((score - fetchedData[0].assessmentScore) /
                Math.max(
                  extractedQAAssessment.length - fetchedData[0].assessmentScore,
                  1
                )) *
              100
            ).toFixed(2)
          ...
          let data = {
            ...
            assessmentScore: score,
            assessmentImp:
              parseInt(score) === parseInt(fetchedData[0].assessmentScore)
                ? 100
                : validImprovement,
            assessmentScorePerf: (
              (score / extractedQAAssessment.length) *
          ...
          if (
            newlyFetchedDashboardDataValues.length > 0 &&
            newlyFetchedDashboardDataValues[0].assessmentScore !== "none"
          ) {
            if (newlyFetchedDashboardDataValues.length >= 2) {
              setLastAssessmentScore(
                newlyFetchedDashboardDataValues[1].assessmentScore
              );
              setAssessmentCountMoreThanOne(true);
            }
          }
    ...
    if (isSubmittedButtonClicked) {
      ...
      socket.emit("updated_score", {
        room: assessementRoom,
        assessmentScore: score,
      });
      socket.emit("updated_isSubmitted_assess", {
        room: assessementRoom,
        isSubmittedChar: true,
      });
      socket.emit("updated_isAssessment_done", {
        room: assessementRoom,
        isAssessmentDone: true,
      });
      socket.emit("updated_running", {
        room: assessementRoom,
        isRunning: false,
      });
  ...
  const submitAnswer = () => {
    if (seconds <= 0 && isRunning) {
      ...
      const hasNullValues = selectedAssessmentAnswer.some(
        (answer) => answer === null
      );

      if (
        !hasNullValues &&
        selectedAssessmentAnswer.length === extractedQAAssessment.length
      ) {
        dataForSubmittingAnswers(userId);
      } else {
  ...
  const generateAnalysis = async (id) => {
    setShowTexts(false);
    socket.emit("updated_show_texts", {
      room: assessementRoom,
      showTexts: false,
    });

    const generateAnalysisUrl =
      "https://46c4-34-67-190-148.ngrok.io/generate_analysis";

    let predictionText =
      overAllPerformance.toFixed(2) >= 90 ? "ready" : "not yet ready";
    ...
    if (fetchedData.length === 1) {
      ...
      setAssessmentCountMoreThanOne(false);
      socket.emit("updated_assessment_count_more_than_one", {
        room: assessementRoom,
        assessmentCountMoreThanOne: false,
      });
    ...
    socket.emit("updated_generated_analysis", {
      room: assessementRoom,
      generatedAnalysis: generatedAnalysisResponse,
    });
    ...
    socket.emit("updated_show_analysis", {
      room: assessementRoom,
      showAnalysis: true,
    });
    socket.emit("updated_show_assessment", {
      room: assessementRoom,
      showAssessment: false,
    });
    socket.emit("updated_show_submitted_answer_modal", {
      room: assessementRoom,
      showSubmittedAnswerModal: false,
    });

    socket.emit("updated_over_all_items", {
      room: assessementRoom,
      overAllItems: newlyFetchedDashboardDataValues.overAllItems,
    });

    socket.emit("updated_pre_assessment_score", {
      room: assessementRoom,
      preAssessmentScore: newlyFetchedDashboardDataValues.preAssessmentScore,
    });

    socket.emit("updated_assessment_score_latest", {
      room: assessementRoom,
      assessmentScoreLatest: newlyFetchedDashboardDataValues.assessmentScore,
    });

    socket.emit("updated_assessment_imp", {
      room: assessementRoom,
      assessmentImp: newlyFetchedDashboardDataValues.assessmentImp,
    });

    socket.emit("updated_assessment_score_perf", {
      room: assessementRoom,
      assessmentScorePerf: newlyFetchedDashboardDataValues.assessmentScorePerf,
    });

    socket.emit("updated_completion_time", {
      room: assessementRoom,
      completionTime: newlyFetchedDashboardDataValues.completionTime,
    });
    socket.emit("updated_confidence_level", {
      room: assessementRoom,
      confidenceLevel: newlyFetchedDashboardDataValues.confidenceLevel,
    });
    socket.emit("updated_over_all_performance", {
      room: assessementRoom,
    ...
    if (isRunning && seconds > 0) {
      interval = setInterval(() => {
        ...
        setSeconds((prevSeconds) => {
          ...
          socket.emit("update_time", {
            room: assessementRoom,
            timeDurationVal: updatedSeconds,
          });
  ...
  const startAssessmentBtn = async () => {
    ...
    setIsRunning(true);
    socket.emit("updated_assessment_started", {
      isStarted: true,
      assessementRoom,
    });
  ...
  const leaveAssessmentPageRoom = () => {
    const userIndex = userListAssessment.findIndex(
    ...
    socket.emit("leave-assessment-page-room", {
      assessementRoom,
    ...
    <div className="poppins mbg-100 mcolor-900 min-h-[100vh]" id="currSec">
      {toggledChatInfo === "hidden" && (
        <div
          ...
          } fixed bottom-5 right-5 btn-primary p-3 rounded-full`}
          style={{ zIndex: 60 }}
        >
          <button onClick={toggleChatInfo}>
            <TextsmsRoundedIcon sx={{ fontSize: "30px" }} />
          </button>
        </div>
      )}
      ...
      {showAssessment && (
        <div className="flex justify-betweeen">
          ...
          <div
            className={`${
              extraLargeDevices || largeDevices
                ? "w-1/2 relative"
                : `${smallDevice || mediumDevices ? "w-2/3" : "w-full"}`
            } ${
              !extraLargeDevices && !largeDevices
                ? `fixed ${toggledChatInfo}`
                : ""
            } mbg-100 shadows rounded min-h-[100vh]`}
            style={{ zIndex: 50 }}
          >
            <div
              className={`${
                (extraLargeDevices || largeDevices) && "hidden"
              } w-full flex items-center justify-end pr-4 pt-3`}
              style={{ zIndex: 60 }}
            >
              <button onClick={toggleChatInfo}>
            ...
            <div className="flex justify-center">
              <div
                className={`h-[65vh] rounded fixed ${
                  extraLargeDevices || largeDevices
                    ? "w-1/3"
                    : `${
                        smallDevice || mediumDevices
                          ? "w-2/3 px-3"
                          : "w-full px-3"
                      }`
                }`}
              >
                <div>
                  <div
                    className={`flex ${
                      extraSmallDevice || smallDevice
                        ? "flex-col justify-center items-center text-lg"
                        : `${
                            largeDevices ? "text-xs" : "text-md"
                          } flex-row justify-between items-center pt-5`
                    } w-full pb-4`}
                  >
                    <p className="font-medium">
                      <i class="fa-regular fa-user ml-1 mr-2"></i>
                      <span className="font-bold text-emerald-500">
                        {userListAssessment[0]?.username === username
                          ? "You"
                          : userListAssessment[0]?.username}
                      </span>{" "}
                      - host
                    </p>
                    <div
                      className={`flex items-center gap-3 ${
                        extraSmallDevice || smallDevice
                          ? `mt-5 w-full ${
                              smallDevice ? "flex-row" : "flex-col"
                            }`
                          : ""
                      }`}
                    >
                      <button
                        className={`mbg-200 px-2 py-2 rounded border-thin-800 ${
                          extraSmallDevice || smallDevice
                            ? `${
                                smallDevice
                                  ? isRunning
                      ...
                      >
                        ...
                        <span className="ml-1 bg-red mcolor-100 px-2 rounded-full">
                          {userListAssessment.length >= 1000
                            ? `${(userListAssessment.length / 1000).toFixed(
                                0
                              )}k`
                            : userListAssessment.length}
                        </span>
                      </button>
                      {isStartAssessmentButtonStarted && (
                        <button
                          className={`bg-red mcolor-100 px-4 py-2 rounded ${
                            extraSmallDevice || smallDevice
                              ? `${smallDevice ? "w-1/2" : "w-full"}`
                              : ""
                          }`}
                          onClick={leaveAssessmentPageRoom}
                        >
                          Leave Room
                        </button>
                      )}
                  ...
                  <div
                    className={`${hideModal} absolute top-0 left-0 modal-bg w-full h-full`}
                  >
                    <div className="flex items-center justify-center h-full">
                      <div
                        className={`mbg-100 max-h-[60vh] ${
                          extraLargeDevices || largeDevices
                            ? "w-1/3"
                            : mediumDevices
                            ? "w-1/2"
                            ...
                            ? "w-2/3"
                            : "w-full mx-2"
                        } z-10 relative p-10 rounded-[5px] flex items-center justify-center`}
                      ...
                      >
                        ...
                        <div className={`w-full`}>
                          <p className="text-center text-2xl font-medium mcolor-800 mb-5">
                            Connected Users:
                          </p>
                          ...
                          <ul className="grid grid-results col-6 gap-5">
                            {userListAssessment.map((user) => (
                              <li
                                key={user?.userId}
                                className="shadows flex items-center justify-center py-4 rounded"
                              >
                                <p>
                ...
                <div
                  className={`relative border-medium-700 rounded ${
                    extraLargeDevices || largeDevices
                      ? isRunning
                        ? "h-[78vh]"
                        : "h-[85vh]"
                ...
                >
                  <div className="mbg-700 flex items-center justify-between px-5">
                    <p className="py-2 mcolor-100">
                      <FiberManualRecordIcon className="text-light-green" />{" "}
                      Live Chat
                    </p>
                  </div>
                  <div
                    className={`chat-body h-full pb-20`}
                    style={{ overflowY: "auto" }}
                  >
                  ...
                  <div
                    className="w-full mbg-input absolute bottom-0 right-0 rounded flex items-center"
                    style={{ zIndex: 80, borderTop: "2px solid #627271" }}
                  >
                    ...
                    <button
                      onClick={sendMessage}
                      className="w-1/6 mbg-700 py-2 mcolor-100"
                    >
                      &#9658;
                    </button>{" "}
                    {/* You can add your CSS class for button styling */}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div
            className={`${
              extraLargeDevices || largeDevices ? "w-3/4" : "w-full"
            } px-10 py-10`}
            ...
            <div className="flex items-center justify-center opacity-50">
              <p
                className={`text-center ${
                  extraSmallDevice ? "text-xs w-full px-5" : "text-sm w-3/4"
                }`}
              >
                Only the host can start the assessment, and select or input the
                answers collectively decided upon by the group.
              </p>
            </div>
                ...
                {userListAssessment[0]?.userId === userId && (
                  <div className={`flex items-center justify-center w-full`}>
                    <button
                      ...
                      disabled={successfullyInviting || successfullyInvited}
                      onClick={() => inviteMembers("assessment")}
                    >
                      {successfullyInviting
                ...
                <div
                  className={`flex ${
                    extraSmallDevice || smallDevice
                      ...
                      : "text-md flex-row justify-between items-center"
                  } mt-8`}
                >
                  <p className={`text-center mcolor-700 py-3`}>
                  ...
                  >
                    {userListAssessment &&
                      userListAssessment.length > 1 &&
                      userListAssessment[0] &&
                      userListAssessment[0].userId === userId && (
                        <button
                          className="mbg-700 mcolor-100 px-4 py-2 rounded"
                          onClick={startAssessmentBtn}
                        >
                          Start Assessment
                        </button>
                      )}
                    ...
                    {userListAssessment &&
                      userListAssessment.map((user) => (
                        <li
                          key={user?.userId}
                          className={`text-center ${
                            user?.userId === userListAssessment[0]?.userId
                              ? "mbg-700 mcolor-100"
                              : "mcolor-900 "
                          } shadows p-5 rounded`}
                        >
              ...
              <div>
                <p
                  className={`text-center ${
                    extraLargeDevices || largeDevices
                      ...
                      ? "text-xl"
                      : "text-lg"
                  } font-medium mcolor-700 pt-10`}
                >
                  Assessment for {materialTitle} of {materialCategory}
                </p>

                {!showAnalysis && isAssessmentDone && (
                  <div>
                    <div>
                      <p className="text-center mcolor-500 font-medium mb-8 text-xl">
                        Your score is:{" "}
                      </p>
                      <p className="text-center text-6xl font-bold mcolor-800 mb-20">
                        {score}/{extractedQAAssessment.length}
                      </p>
                      ...
                      <div className=" flex items-center justify-center gap-5">
                        ...
                        {groupId !== undefined ? (
                          <button
                            className="border-thin-800 px-5 py-3 rounded-[5px] w-1/4 text-center"
                            onClick={() => {
                              window.location.reload();
                            }}
                          >
                          ...
                          <Link
                            to={`/main/personal/study-area/personal-review/${materialId}`}
                            className="border-thin-800 px-5 py-3 rounded-[5px] w-1/4 text-center"
                          >
                ...
                {isRunning && (
                  <div
                    className={`${
                      extraSmallDevice
                        ? "text-center text-xs px-5"
                        : "text-sm px-10"
                    } timer-container mbg-input pt-1 pb-1`}
                    style={{ zIndex: 55 }}
                  >
                    <div
                      className="rounded-[5px]"
                      style={{ height: "8px", backgroundColor: "#B3C5D4" }}
                    >
                      <div
                        className="rounded-[5px]"
                        style={{
                          width: `${
                            (seconds / (extractedQAAssessment.length * 60)) *
                            100
                          }%`,
                          height: "100%",
                          backgroundColor:
                            seconds <= 10 ? "#af4242" : "#667F93",
                        }}
                      />
                    </div>
                    ...
                    <h1 className="mcolor-900 text-sm pt-1">
                      Remaining time:{" "}
                      {hours > 0 &&
                        String(hours).padStart(2, "0") + " hours and "}
                      {(hours > 0 || minutes > 0) &&
                        String(minutes).padStart(2, "0") + " mins and "}
                ...
                {Array.isArray(extractedQAAssessment) &&
                  shuffledChoicesAssessment.length > 0 &&
                  extractedQAAssessment.map((item, index) => (
                    <div key={index} className="mb-20">
                      <p
                        className={`mcolor-900 text-justify ${
                          extraLargeDevices || largeDevices
                            ? "text-lg"
                            : mediumDevices || smallDevice
                            ? "text-sm"
                      ...
                      <ul
                        className={`grid ${
                          extraSmallDevice ? "grid-cols-1" : "grid-cols-2"
                        } gap-4 mcolor-800`}
                      >
                        {item.quizType === "MCQA" &&
                          shuffledChoicesAssessment[index].map(
                            (choice, choiceIndex) =>
                              userListAssessment &&
                              userListAssessment.length > 0 &&
                              userListAssessment[0] &&
                              userListAssessment[0]?.userId === userId ? (
                                <div
                                  key={choiceIndex}
                                ...
                                >
                                  <div className="flex items-center justify-center">
                                    <input
                                      ...
                                      onClick={() =>
                                        handleRadioChange(choice, index)
                                      }
                                      checked={
                                        selectedAssessmentAnswer[index] ===
                                        choice
                                    ...
                                    <div className={`flex items-center `}>
                                      <label
                                        htmlFor={`choice-${choiceIndex}-${index}`}
                                        className={`mr-5 pt-1 cursor-pointer ${
                                          extraLargeDevices || largeDevices
                                            ? "text-lg"
                                            : mediumDevices || smallDevice
                                            ? "text-sm"
                      ...
                      {item.quizType === "ToF" && (
                        <div className="grid-result gap-4 mcolor-800">
                          ...
                          ${
                            isSubmitted &&
                            extractedQAAssessment[index].answer === "True"
                              ? "border-thin-800-correct"
                              : isSubmitted &&
                                selectedAssessmentAnswer[index] !==
                                  extractedQAAssessment[index].answer &&
                                selectedAssessmentAnswer[index] === "True"
                              ? "border-thin-800-wrong"
                              : "border-thin-800"
                          }`}
                            >
                          ...
                          ${
                            isSubmitted &&
                            extractedQAAssessment[index].answer === "False"
                              ? "border-thin-800-correct"
                              : isSubmitted &&
                                selectedAssessmentAnswer[index] !==
                                  extractedQAAssessment[index].answer &&
                                selectedAssessmentAnswer[index] === "False"
                              ? "border-thin-800-wrong"
                              : "border-thin-800"
                          }`}
                            >
                      ...
                      {(item.quizType === "Identification" ||
                        item.quizType === "FITB") && (
                        <div>
                          <input
                            className={`mb-5 w-full px-5 py-5 ${
                              extraLargeDevices || largeDevices
                                ? "text-lg"
                                : mediumDevices || smallDevice
                                ? "text-sm"
                                : "text-xs"
                            } quicksand font-bold text-center choice rounded-[5px] box-shadoww ${
                              isSubmitted &&
                              selectedAssessmentAnswer[index] &&
                              extractedQAAssessment[index] &&
                              selectedAssessmentAnswer[index].toLowerCase() ===
                                extractedQAAssessment[
                                  index
                                ].answer.toLowerCase()
                                ? "border-thin-800-correct"
                            ...
                            onChange={(event) =>
                              handleRadioChange(event.target.value, index)
                            }
                            disabled={
                              userListAssessment &&
                              userListAssessment.length > 0 &&
                          ...
                          <p
                            className={`correct-color text-center ${
                              extraLargeDevices || largeDevices
                                ? "text-lg"
                                : mediumDevices || smallDevice
                                ? "text-sm"
                          ...
                          >
                            ...
                            extractedQAAssessment[index] &&
                            selectedAssessmentAnswer[index].toLowerCase() !==
                              extractedQAAssessment[index].answer.toLowerCase()
                              ? extractedQAAssessment[index].answer
                              : null}
                          </p>
                        </div>
                ...
                {!showAnalysis &&
                  ...
                  userListAssessment[0]?.userId === userId && (
                    <div className="flex justify-center pt-3 mb-10">
                      <button
                        className={`w-1/2 py-2 px-5 rounded-[5px] text-lg ${
                          isSubmittedButtonClicked &&
                          idOfWhoSubmitted !== userId &&
                          usernameOfWhoSubmitted !== username
                            ? "disabled-button mbg-300 mcolor-800"
                            : "mbg-800 mcolor-100 "
                        }`}
                        onClick={() => {
                          socket.emit("submitted_button_clicked", {
                            room: assessementRoom,
                          ...
                          if (
                            ...
                            socket.emit("updated_id_of_who_submitted", {
                              room: assessementRoom,
                              idOfWhoSubmitted: userId,
                            });
                            socket.emit("updated_username_of_who_submitted", {
                              room: assessementRoom,
                              usernameOfWhoSubmitted: username,
                            });
                      ...
                      >
                        {isSubmittedButtonClicked
                          ? idOfWhoSubmitted === userId
                            ? "Confirm Submission"
                            : `${usernameOfWhoSubmitted} clicks the submit button`
                          : "SUBMIT"}
                      </button>
                    </div>
                  )}
                  ...
                  userListAssessment[0]?.userId !== userId && (
                    <div className="flex justify-center pt-3 mb-10">
                      <button
                        className={`${
                          isSubmittedButtonClicked &&
                          `w-1/2 py-2 px-5 rounded-[5px] text-lg ${
                            isSubmittedButtonClicked &&
                            idOfWhoSubmitted !== userId &&
                            usernameOfWhoSubmitted !== username
                              ? "disabled-button mbg-300 mcolor-800"
