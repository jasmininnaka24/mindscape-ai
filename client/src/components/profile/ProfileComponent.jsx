...
export const ProfileComponent = () => {
  ...
  const [contributedMaterialsLength, setContributedMaterialsLength] =
    useState(0);

  const [showAccountDeletionInputPass, setShowAccountDeletionInputPass] =
    useState(false);
  ...
  const fetchUserDataFrontend = async () => {
    try {
      const userData = await fetchUserData(
        !userId || UserId === parseInt(userId, 10) ? UserId : userId
      );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let totalGroups = 0;
      const groupListLengthResponse = await axios.get(
        `${SERVER_URL}/studyGroup/extract-group-through-user/${
          !userId || UserId === parseInt(userId, 10) ? UserId : userId
        }`
      );

      const groupListMemberLengthResponse = await axios.get(
        `${SERVER_URL}/studyGroupMembers/get-materialId/${
          !userId || UserId === parseInt(userId, 10) ? UserId : userId
        }`
      );
      ...
      const contributedMaterialsResponse = await axios.get(
        `${SERVER_URL}/studyMaterial/shared-materials-by-userid/${
          !userId || UserId === parseInt(userId, 10) ? UserId : userId
        }`
      );
      ...
      const fetchPersonalStudyMaterial = async () => {
        const personalStudyMaterial = await axios.get(
          `${SERVER_URL}/studyMaterial/study-material-category/Personal/${
            !userId || UserId === parseInt(userId, 10) ? UserId : userId
          }`
        );
      ...
      const fetchGroupStudyMaterial = async () => {
        const groupStudyMaterial = await axios.get(
          `${SERVER_URL}/studyMaterial/study-material-category/Group/${
            !userId || UserId === parseInt(userId, 10) ? UserId : userId
          }`
        );
      ...
      const fetchSharedStudyMaterial = async () => {
        const sharedStudyMaterial = await axios.get(
          `${SERVER_URL}/studyMaterial/shared-materials`
        );
      ...
      const fetchSharedMaterialCategory = async () => {
        ...
        const fetchedSharedStudyMaterialCategory = await Promise.all(
          sharedStudyMaterialResponse.map(async (material, index) => {
            const materialCategorySharedResponse = await axios.get(
              `${SERVER_URL}/studyMaterialCategory/shared-material-category/${
                material.StudyMaterialsCategoryId
              }/Group/${
                !userId || UserId === parseInt(userId, 10) ? UserId : userId
              }`
            );
        ...
        const uniqueCategories = [
          ...new Set(
            fetchedSharedStudyMaterialCategory.map((item) => item.category)
          ),
        ];
        ...
        const promiseArray = sortedCategories.map(async (category) => {
          const firstOccurrence = fetchedSharedStudyMaterialCategory.find(
      ...
      const fetchSharedMaterialDetails = async () => {
        ...
        const sortedDataUsers = await Promise.all(
          sharedStudyMaterialResponse.map(async (user) => {
            const response = await axios.get(
              `${SERVER_URL}/users/get-user/${user.UserId}`
            );
        ...
        const sortedDataBookmarksCounts = await Promise.all(
          sharedStudyMaterialResponse.map(async (user) => {
            const response = await axios.get(
              `${SERVER_URL}/studyMaterial/bookmark-counts/${user.code}`
            );
            return response.data;
          })
        );
      ...
      const fetchGroups = async () => {
        ...
        if (response.data.length === 0) {
          ...
          const materialPromises = userMemberGroupList.data.map(
            async (item) => {
      ...
      const filteredPersonalStudyMaterials = personalStudyMaterial.filter(
        (item) => item.tag === "Own Record"
      );
      ...
      const fetchedPersonalStudyMaterialCategory = await Promise.all(
        personalStudyMaterial.map(async (material, index) => {
          const materialCategoryResponse = await axios.get(
            `${SERVER_URL}/studyMaterialCategory/get-categoryy/${material.StudyMaterialsCategoryId}`
          );
      ...
      const filteredGroupStudyMaterials = groupStudyMaterial.filter(
        (item) => item.tag === "Own Record"
      );
  ...
  const updateUserImage = async (e, buttonclickedNum) => {
    ...
    if (image) {
      ...
      try {
        ...
        await axios
          .put(
            `${SERVER_URL}/users/update-user-image/${
              !userId || UserId === parseInt(userId, 10) ? UserId : userId
            }`,
            data
          )
  ...
  if (loading) {
    ...
    return (
      <div className="poppins mcolor-900 mbg-200 relative flex">
        ...
        <Sidebar
          currentPage={
            (!userId || UserId === parseInt(userId, 10)) && "profile"
          }
        />
        ...
        >
            ...
            <div className="mbg-100 p-4 rounded-[1rem]">
              ...
              >
                <motion.button
                  ...
                  whileTap="active"
                  className={`${
                    showMainProfile ? "btn-800 mcolor-100" : "mcolor-800"
                  } text-center rounded-[1rem] w-full lg:w-full lg:mr-2 py-2`}
                  onClick={() => {
                ...
                <motion.button
                  ...
                  whileTap="active"
                  className={`${
                    showAccountSettings ? "btn-800 mcolor-100" : "mcolor-800"
                  } text-center rounded-[1rem] w-full lg:w-full lg:mr-2 py-2`}
                  onClick={() => {
                    setShowMainProfile(false);
                    setShowPasswordSecurity(false);
                ...
                <motion.button
                  ...
                  whileTap="active"
                  className={`${
                    showPasswordSecurity ? "btn-800 mcolor-100" : "mcolor-800"
                  } text-center rounded-[1rem] w-full lg:w-full lg:mr-2 py-2`}
                  onClick={() => {
                    setShowAccountSettings(false);
                    setShowMainProfile(false);
                ...
                <motion.button
                  ...
                  whileTap="active"
                  className={`${
                    showAccountDeletion ? "btn-800 mcolor-100" : "mcolor-800"
                  } text-center rounded-[1rem] w-full lg:w-full lg:mr-2 py-2`}
                  onClick={() => {
                    setShowAccountSettings(false);
                    setShowMainProfile(false);
          ...
          >
            <div
              className={`${
                extraLargeDevices || largeDevices ? "w-2/3 pr-5" : "w-full mt-5"
              }`}
            >
              {showMainProfile && (
                <motion.div
                ...
                >
                  ...
                  >
                    ...
                    <br />
                    <br />
                    <p className="text-xl mcolor-800 font-medium">
                    ...
                    >
                      ...
                      <li className="w-full">
                        ...
                        <input
                          type="text"
                          disabled
                          className="border-medium-800 w-full py-2 rounded mbg-200 px-5"
                      ...
                      <li className="w-full">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setUserData({
                              ...userData,
                              studyProfTarget: event.target.value,
                            }) || ""
                          }
                        >
                      ...
                      <li className="w-full">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setUserData({
                              ...userData,
                              typeOfLearner: event.target.value,
                            }) || ""
                          }
                        >
                          ...
                          {/* Other options excluding the default value */}
                          {["Auditory", "Visual", "Kinesthetic"].map(
                            (option) =>
                    ...
                    <div className="flex items-center justify-end w-full mt-10">
                      <button
                        className={`${
                          ...
                          extraSmallDevice ? "w-full" : ""
                        }`}
                        disabled={buttonLoading && buttonClickedNumber === 2}
                        onClick={(e) => updateUserInformation(e, 2)}
                      >
                        {buttonLoading && buttonClickedNumber === 2 ? (
                          <div>Updating...</div>
                        ) : (
                  ...
                  >
                    <div>
                      ...
                      {contributedMaterialsLength > 0 ? (
                        ...
                        <p className="mb-3 text-lg text-center mcolor-500">
                          No Contributed Materials
                        </p>
                      )}
                        ...
                        {sharedMaterials
                          .map((material, index) => {
                            ...
                            const user =
                              ...
                              "Deleted user";
                            const userIdMaterial =
                              material?.UserId || "Deleted user";
                            const bookmarks =
                          ...
                          .sort(
                            (a, b) =>
                              b.latestBookmarkTimestamp -
                              a.latestBookmarkTimestamp
                          )
                          .map(
                            ({
                              index,
                    ...
                    {showModal && (
                      ...
                      >
                        ...
                        >
                          <div
                            className={`relative mbg-100 min-h-[30vh] max-h-[80vh] ${
                              extraLargeDevices || largeDevices
                                ? "w-1/2"
                                : mediumDevices || smallDevice
                                ? "w-2/3"
                                : "w-full mx-2"
                            } z-10 relative py-5 px-10 rounded-[5px]`}
                          ...
                          >
                            ...
                            {showPresentStudyMaterials && (
                              <div>
                                <p class="text-lg text-color-900 font-medium mb-5 mt-10 mcolor-900 text-center">
                                  Materials that are present in your personal
                                  room and those you contributed within
                                ...
                                <div className="flex items-center justify-center mb-2">
                                  <Link to={"/main/library/generate-quiz"}>
                                    <button className="btn-800 mcolor-100 px-5 py-2 rounded border-thin-800">
                                ...
                                <p>Personal: </p>
                                {personalStudyMaterials.map(
                                  (material, index) => {
                                    const category =
                                      personalStudyMaterialsCategory[index]
                                        ?.category || "Category not available";

                                    return (
                                      <div
                                        key={index}
                                        className="my-3 mbg-200 border-thin-800 p-4 rounded flex items-center justify-between"
                                      >
                                        <div>
                                        ...
                                        <div className="flex items-center gap-3">
                                          <button
                                            className="mbg-100 mcolor-900 border-thin-800 px-5 py-2 rounded"
                                            onClick={() =>
                                              viewStudyMaterialDetails(
                                                index,
                                                "personal",
                                ...
                                {groupStudyMaterials.map((material, index) => {
                                  const category =
                                    groupStudyMaterialsCategory[index]
                                      ?.category || "Category not available";
                ...
                >
                  ...
                  >
                    <form className="px-8 gap-8 py-5">
                      <p className="text-2xl mcolor-800 font-medium mb-5">
                        Resetting Password
                      </p>
                      <p className="mcolor-800">
                        Begin the password reset process by entering your email
                        address below. Our system will quickly initiate the
                        necessary steps to secure your account. Once submitted,
                        a confirmation email will be sent to the provided
                        address, containing instructions on how to efficiently
                        reset your password.
                      </p>
                      ...
                      <div className="flex items-center justify-end mt-5">
                        <Link
                          to={"/verify-email"}
                          className=" py-2 px-5 btn-primary rounded"
                        >
                          Reset Password
                        </Link>
                      </div>
                  ...
                  >
                    <div className="shadows p-5 mbg-100 rounded mb-5">
                      <form className="px-8 gap-8 py-5">
                        ...
                        <p className="mcolor-800">
                          Keep in mind that this action is irreversible and will
                          result in the removal of all your information from our
                          system.
                        </p>
            ...
            >
              ...
              >
                <p className="text-2xl font-medium mb-5 mt-3">
                  {userData.username}
                </p>
