...
export const LibraryComponent = () => {
  ...
  const [personalStudyMaterialsCategory, setPersonalStudyMaterialsCategory] =
    useState([]);
  const [groupStudyMaterials, setGroupStudyMaterials] = useState([]);
  const [groupStudyMaterialsCategory, setGroupStudyMaterialsCategory] =
    useState([]);

  const [sharedMaterials, setSharedMaterials] = useState([]);
  const [sharedMaterialsCategory, setSharedMaterialsCategory] = useState([]);
  const [sharedMaterialsCategoryUsers, setSharedMaterialsCategoryUsers] =
    useState([]);
  const [
  ...
  const [currentSharedMaterialIndex, setCurrentSharedMaterialIndex] =
    useState(0);
  ...
  const [currentFilteredCategoryUsers, setCurrentFilteredCategoryUsers] =
    useState("");
  const [
  ...
  const [searchedMaterialsCategories, setSearchedMaterialsCategories] =
    useState([]);
  ...
  const [showPresentStudyMaterials, setShowPresentStudyMaterials] =
    useState(false);
    ...
    try {
      ...
      const fetchPersonalStudyMaterial = async () => {
        const personalStudyMaterial = await axios.get(
          `${SERVER_URL}/studyMaterial/study-material-category/Personal/${UserId}`
        );
      ...
      const fetchGroupStudyMaterial = async () => {
        const groupStudyMaterial = await axios.get(
          `${SERVER_URL}/studyMaterial/study-material-category/Group/${UserId}`
        );
        ...
        const fetchedSharedStudyMaterialCategory = await Promise.all(
          sharedStudyMaterialResponse.map(async (material, index) => {
            const materialCategorySharedResponse = await axios.get(
              `${SERVER_URL}/studyMaterialCategory/shared-material-category/${material.StudyMaterialsCategoryId}/Group/${UserId}`
            );
  ...
  const viewStudyMaterialDetails = async (
    index,
    materialFor,
    filter,
    category
    ...
    try {
      const revResponse = await axios.get(
        `${SERVER_URL}/quesRev/study-material-rev/${materialId}`
      );
  ...
  const shareMaterial = async (index, materialFor, materialTitle) => {
    ...
    try {
      ...
      if (materialArray && materialArray.length > index) {
        ...
        await axios.put(
          `${SERVER_URL}/studyMaterial/update-tag/${materialId}`,
          { tag: "Shared" }
        );
        ...
        if (currentMaterialCategory !== "") {
          ...
          const newSharedMaterial = await axios.get(
            `${SERVER_URL}/studyMaterial/get-material/${studyMaterialResponse.data.id}`
          );

          // Update the state with the new material added to the beginning of the array
          setFilteredStudyMaterialsByCategory((prevMaterials) => [
            newSharedMaterial.data,
            ...prevMaterials,
          ]);
        }
      } else {
        console.error(
          `Material at index ${index} is undefined or out of bounds.`
        );
      }
    } catch (error) {
      fetchData();
  ...
  const bookmarkMaterial = async (index, id, materialFor, buttonclickedNum) => {
    ...
    const savedFunctionality = async () => {
      ...
      try {
        let mcqResponse = await axios.get(
          `${SERVER_URL}/quesAns/study-material-mcq/${sharedMaterials[index].id}`
        );
        ...
        genIdentification = genQAData.filter(
          (data) => data.quizType === "Identification"
        );
        ...
        if (Array.isArray(genMCQAData)) {
          const materialChoices = genMCQAData.map(async (materialChoice) => {
            try {
              let choiceResponse = await axios.get(
                `${SERVER_URL}/quesAnsChoices/study-material/${sharedMaterials[index].id}/${materialChoice.id}`
              );
      ...
      try {
        let revResponse = await axios.get(
          `${SERVER_URL}/quesRev/study-material-rev/${sharedMaterials[index].id}`
        );
    ...
    if (materialFor === "Personal") {
      ...
      const filteredMaterialResult = bookmarkedPersonalMaterial.filter(
        (material) =>
          material.code === materialCode &&
          material.materialFor === materialFor &&
  ...
  const filterMaterial = async (categoryTitle) => {
    try {
      const filteredCategory = sharedMaterialsCategory.filter(
      ...
      const matchingMaterials = sharedMaterials.filter((material) =>
        filteredUniqueCategory.some(
          (categoryItem) =>
            categoryItem.id === material.StudyMaterialsCategoryId
        )
      );
    ...
    try {
      ...
      const filteredMaterials = sharedMaterials.filter((material) => {
        ...
        return (
          ...
          lowercaseBody.includes(lowercaseValue)
        );
      });
      ...
      const matchingMaterials = filteredUniqueCategory.filter((biologyItem) =>
        sharedMaterials.some(
          (material) => biologyItem.id === material.StudyMaterialsCategoryId
        )
      );

      // Use Promise.all with map for asynchronous operations
      const [
        fetchedSharedStudyMaterialCategory,
        fetchedSearchCategoryMaterials,
      ] = await Promise.all([
        Promise.all(
        ...
        Promise.all(
          filteredCategories.map(async (material) => {
            const sharedStudyMaterial = await axios.get(
              `${SERVER_URL}/studyMaterial/shared-materials/${material.id}`
            );
            return sharedStudyMaterial.data;
          })
        ),
  ...
  const removeFromLibraryOnly = async () => {
    ...
    try {
      ...
      await axios.put(
        `${SERVER_URL}/studyMaterial/update-tag/${materialIdToRemove}`,
        data
      );

      const materialIndexToRemove = filteredStudyMaterialsByCategory.findIndex(
        (material) => material.id === materialIdToRemove
      );
      ...
      if (materialIndexToRemove !== -1) {
        ...
        console.error(
          `Material with ID ${materialIdToRemove} not found in filteredStudyMaterialsByCategory.`
        );
      }
  ...
  const deleteInAllRecords = async () => {
    ...
    try {
      await axios.delete(
        `${SERVER_URL}/studyMaterial/delete-material/${materialIdToRemove}`
      );

      const updatedFilteredStudyMaterials =
        filteredStudyMaterialsByCategory.filter(
          (material) => material.id !== materialIdToRemove
        );
