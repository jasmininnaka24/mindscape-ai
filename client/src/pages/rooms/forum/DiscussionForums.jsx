...
export const DiscussionForums = () => {
  ...
  const [discussionForumAllActiveUsers, setDiscussionForumAllActiveUsers] =
    useState([]);
  ...
  const filteredRooms = roomList.filter(
    (room) =>
      (!selectedCategory ||
        room?.roomCategory?.toLowerCase() === selectedCategory.toLowerCase()) &&
  ...
  const uniqueCategories = [
    ...new Set(
      roomList.map((room) => capitalizeFirstLetter(room?.roomCategory))
    ),
  ];
  ...
  const leaveDiscussionForumRoom = async (room) => {
    ...
    const userIndex = roomList[roomIndex].users.findIndex(
    ...
    const updatedUsers = updatedRoomList[roomIndex].users.filter(
    ...
    if (updatedUsers.length === 0) {
      updatedRoomList = updatedRoomList.filter(
        (updatedRoom) => updatedRoom.room !== room
      );
    }

    setRoomList(updatedRoomList);
    console.log(updatedRoomList);
    socket.emit("leave-discussion-forum-room", {
      room: discussionForumRoom,
  ...
  return (
    <div className="poppins mcolor-900 mbg-200 relative flex">
      ...
      <div
        className={`min-h-[100vh] flex flex-col items-center justify-between py-2 ${
      ...
      {showRooms && (
        <div
          className={`flex-1 mbg-200 w-full pt-5 ${
            extraSmallDevice ? "px-3" : "px-8"
          } min-h-[100vh]`}
        >
          <div>
            ...
            <div
              className={`flex ${
                extraSmallDevice || smallDevice ? "flex-col" : "flex-row"
              } items-center justify-between`}
            >
              <p
                className={`${
                  extraSmallDevice || smallDevice ? "text-xl" : "text-3xl"
                } font-medium flex items-center mcolor-900`}
              >
                <GroupsIcon
                  className="mr-3 mcolor-700"
                  sx={{ fontSize: extraSmallDevice || smallDevice ? 25 : 45 }}
                />
              ...
              {filteredRooms.length !== 0 ? (
                <div
                  className={`${
                    (extraSmallDevice || smallDevice) && "mt-4"
                  } rounded`}
                >
                  <button
                    className={`px-6 my-1 py-2 ${
                      extraSmallDevice || smallDevice ? "text-sm" : "text-md"
                    } mbg-input mcolor-800 shadows rounded`}
                    onClick={() => {
                      setShowrooms(false);
            ...
            <div className="w-full flex items-center justify-center gap-5 mt-3">
              <div
                className={`flex ${
                  extraSmallDevice || smallDevice ? "flex-col mt-5" : "flex-row"
                } items-center gap-5 w-full`}
              >
                <div
                  className={`flex items-center justify-center w-full gap-1 ${
                    extraSmallDevice || smallDevice ? "text-sm" : "text-md my-5"
                  } ${filteredRooms.length === 0 && "hidden"}`}
                >
                  <button
                    className="btn-primary px-5 py-2 rounded w-full"
                    onClick={() => {
                      setShowCreateRoomModal(true);
                    }}
                  >
                ...
                >
                  <button
                    className="mbg-800 mcolor-100 border-thin-800 px-5 py-2 rounded w-full"
                    onClick={() => {
            ...
            {showCreateRoomModal && (
              <div className={`absolute top-0 modal-bg left-0 w-full h-full`}>
                <div className="flex items-center justify-center h-full">
                  <div
                    className={`relative mbg-100 h-[70vh] ${
                  ...
                  >
                    ...
                    <p className="text-center text-2xl mcolor-900 mb-5 mt-3">
                      Create a room
                    </p>
                    ...
                    <div className="border-thin-800 w-full rounded flex justify-center relative">
                      ...
                      />
                      <span className="absolute right-2 bottom-1 opacity-50 text-sm">
                        {roomNameValue.length}/20
                      </span>
                    </div>
                    ...
                    <div className="border-thin-800 w-full rounded flex justify-center relative">
                      <input
                        ...
                        style={{ zIndex: 80 }}
                        value={
                          roomCategoryValue !== "" ? roomCategoryValue : ""
                        }
                        onChange={(event) =>
                          setRoomCategoryValue(event.target.value)
                        }
                      />
                      <span className="absolute right-2 bottom-1 opacity-50 text-sm">
                        {roomCategoryValue.length}/20
                      </span>
                    </div>
                    ...
                    <div className="border-thin-800 w-full rounded flex justify-center relative">
                      <textarea
                        ...
                        onChange={(event) =>
                          setRoomDescValue(event.target.value)
                        }
                      ></textarea>
                      <span className="absolute right-2 bottom-1 opacity-50 text-sm">
                        {roomDescValue.length}/150
                      </span>
                    </div>

                    <br />
                    <button
                      className="mbg-700 mcolor-100 px-5 py-2 rounded w-full"
                      onClick={createRoom}
                    >
                      Create a room
                    </button>
            ...
            {(searchTerm !== "" || selectedCategory !== null) && (
              <div className="flex items-center justify-end my-5">
                <button
                  className={`mbg-700 border-thin-800 rounded py-1 px-5 mcolor-100 ${
                ...
                <div className="flex items-center justify-center h-full">
                  ...
                  >
                    ...
                    {uniqueCategories.map((uniqueCategory) => (
                      <button
                        key={uniqueCategory}
                        className={`w-full my-1 mbg-300 border-thin-800 rounded py-1 ${
            ...
            {filteredRooms.length > 0 ? (
              ...
              <p
                className={`mcolor-500 text-center font-medium my-20 ${
                  extraSmallDevice || smallDevice ? "text-sm" : "text-lg"
                }`}
              >
                No available discussion room to join.
              </p>
            )}
            ...
            >
              {filteredRooms
                ...
                .map((room, index) => {
                  const userIdExists = room?.users?.some(
                    (userr) => userr.userId === user?.id
                  );
                  return (
                    <div
                      key={index}
                      className="mbg-input border-thin-800 py-3 px-5 rounded"
                    >
                      {room && (
                        <>
                          <p
                          ...
                          >
                            Room name:{" "}
                            <span className={`font-medium mcolor-900`}>
                              {room?.roomName || "N/A"}
                            </span>
                          </p>
                          <p
        ...
        >
          ...
          >
            ...
            >
                ...
                .map((room, index) => {
                  ...
                  if (userIdExists) {
                    return (
                      <div
                        key={index}
                        className="mbg-input border-thin-800 w-full py-3 px-5 mb-5 rounded"
                      >
                        {room && (
                          <>
                            ...
                            <div className="flex justify-between mt-3">
                              ...
                              <div className="flex items-center gap-3">
                                {roomList &&
                                  roomList[currentIndex]?.roomName !==
                                    room?.roomName && (
                                    <div>
                                      <button
                                ...
                                <button
                                  className="bg-red mcolor-100 px-5 text-sm py-2 rounded"
                                  onClick={() =>
                                    leaveDiscussionForumRoom(room?.room)
                                  }
                                >
                                  Leave Room
                                </button>
              ...
              {filteredRooms.length === 0 && (
                <p className="mcolor-500 text-center font-medium my-8 text-lg">
                  You haven't joined in any room yet
                </p>
              )}
            </div>
            ...
            <div
              className={`border-medium-700 rounded ${
                extraLargeDevices || largeDevices ? "w-1/2" : "w-full"
              } h-[80vh]`}
            >
              <div className="relative h-full">
                <div className="mbg-700 flex items-center justify-between px-5">
                  {roomList &&
                    roomList[currentIndex]?.users?.some(
                      ...
                      <div className="flex items-center justify-between w-full">
                        ...
                        <button
                          className={`mcolor-900 mbg-300 px-4 py-1 my-2 rounded ${
                            extraSmallDevice || smallDevice
                              ? "text-xs"
                              : "text-sm"
                          }`}
                    ...
                    roomList[currentIndex] && (
                      <ScrollToBottom className="message-container">
                        {roomList[currentIndex].messages ? (
                          roomList[currentIndex].messages.map(
                            (messageContent) => (
                              <div
                                className="message"
                                id={
