...
export const GroupReviewerPage = () => {
  ...
  const [isStartStudyButtonStarted, setIsStartStudyButtonStarted] =
    useState(false);
  ...
  const [isSubmittedButtonClicked, setIsSubmittedButtonClicked] =
    useState(false);
  ...
  const [showSubmittedAnswerModal, setShowSubmittedAnswerModal] =
    useState(false);
  ...
  const [assessmentCountMoreThanOne, setAssessmentCountMoreThanOne] =
    useState(false);
  ...
  const [isStartAssessmentButtonStarted, setIsStartAssessmentButtonStarted] =
    useState(false);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const groupResponse = await axios.get(
          `${SERVER_URL}/studyMaterial/study-code/${groupId}/${materialId}`
        );
        const materialResponse = await axios.get(
          `${SERVER_URL}/quesAns/study-material-mcq/${materialId}`
        );
        ...
        const userHostResponse = await axios.get(
          `${SERVER_URL}/users/get-user/${groupResponse.data.UserId}`
        );
              ...
              if (randomNumber % 2 === 0) {
                try {
                  const choicesResponse = await axios.get(
                    `${SERVER_URL}/quesAnsChoices/study-material/${materialId}/${item.id}`
                  );
                  const randomIndex = Math.floor(
                    rng() * choicesResponse.data.length
                  );
        ...
        if (Array.isArray(updatedData)) {
          const shuffledChoicesPromises = updatedData.map(async (item) => {
            try {
              const materialChoicesResponse = await axios.get(
                `${SERVER_URL}/quesAnsChoices/study-material/${materialId}/${item.id}`
              );
              const choices = materialChoicesResponse.data.map(
    ...
    socket.on("received_next_user", (nextUser) => {
      ...
      setCurrentCount(20);
      socket.emit("update_time_review", {
        room: room,
        timeDurationValReview: 20,
      });
    });
    socket.on("received_question_index", (nextQuestion) =>
    ...
    socket.on("study_session_started", (isStarted) =>
      setIsStartStudyButtonStarted(isStarted)
    );
    ...
    if (itemsLength === itemsDone && !doneCheckingScores) {
      const uniqueUserScores = Array.from(
        new Set(userList.map((user) => user.points))
      );
  ...
  const inviteMembers = async (sentence) => {
    ...
    if (sentence === "study session") {
      ...
      updatedSentence =
        "and team up with your co-members to take the assessment";
    }

    const groupMembers = await axios.get(
      `${SERVER_URL}/studyGroupMembers/get-members/${groupId}`
    );
    ...
    for (const member of groupMembersData) {
      ...
      const groupMembersDetails = await axios.get(
        `${SERVER_URL}/users/get-user/${member.UserId}`
      );
      if (groupMembersDetails.data !== null) {
        ...
        if (member.UserId !== userId) {
          await axios.post(
            `${SERVER_URL}/studyGroupMembers/invite-members`,
            invitationData
          );
        }
      }
  ...
  const joinRoom = async () => {
    ...
    socket.emit("join_room", {
      ...
      timeDurationValReview: currentCount,
      extractedQA,
  ...
  const responseStateUpdate = async (questionId, responseStateInp) => {
    ...
    await axios.put(
      `${SERVER_URL}/quesAns/update-response-state/${materialId}/${questionId}`,
      data
    );
  };
  ...
  const otherUserFunctionalities = () => {
    ...
    socket.emit("updated_question_index", {
    ...
    if (Array.isArray(extractedQA) && extractedQA.length > 0) {
      if (extractedQA[questionIndexVal].quizType !== "ToF") {
        failCountDefault(2, "");
      } else {
        failCountDefault(1, "");
      }
  ...
  const submitAnswer = () => {
    if (selectedChoice === extractedQA[questionIndex].answer) {
      setTimeout(() => {
        ...
        socket.emit("update_is_running_review", {
          room,
    ...
    if (remainingHints >= 1) {
      ...
      if (remainingHints === 0) {
        speechSynthesis.speak(
          new SpeechSynthesisUtterance("No more remaining hint")
        );
      }
    ...
    if (
      ...
      interval = setInterval(() => {
        setSecondsReview((prevSeconds) => {
          ...
          socket.emit("update_time_review", {
            room: room,
            timeDurationValReview: updatedSeconds,
          });
          ...
          <div>
            {showPreJoin && (
              <PreJoinPage
                ...
                groupId={groupId}
                setShowPreJoin={setShowPreJoin}
                setShowAssessmentPage={setShowAssessmentPage}
                room={room}
                assessementRoom={assessementRoom}
                username={userData?.username}
                userId={userId}
                userListAssessment={userListAssessment}
                setUserListAssessment={setUserListAssessment}
                ...
                isSubmittedButtonClicked={isSubmittedButtonClicked}
                setIsSubmittedButtonClicked={setIsSubmittedButtonClicked}
                idOfWhoSubmitted={idOfWhoSubmitted}
                setIdOfWhoSubmitted={setIdOfWhoSubmitted}
                ...
                setIsAssessmentDone={setIsAssessmentDone}
                showSubmittedAnswerModal={showSubmittedAnswerModal}
                setShowSubmittedAnswerModal={setShowSubmittedAnswerModal}
                showTexts={showTexts}
                ...
                setOverAllItems={setOverAllItems}
                preAssessmentScore={preAssessmentScore}
                setPreAssessmentScore={setPreAssessmentScore}
                assessmentScore={assessmentScore}
                setAssessmentScore={setAssessmentScore}
                assessmentImp={assessmentImp}
                setAssessmentImp={setAssessmentImp}
                assessmentScorePerf={assessmentScorePerf}
                setAssessmentScorePerf={setAssessmentScorePerf}
                completionTime={completionTime}
                setCompletionTime={setCompletionTime}
                confidenceLevel={confidenceLevel}
                setConfidenceLevel={setConfidenceLevel}
                overAllPerformance={overAllPerformance}
                setOverAllPerformance={setOverAllPerformance}
                assessmentCountMoreThanOne={assessmentCountMoreThanOne}
                setAssessmentCountMoreThanOne={setAssessmentCountMoreThanOne}
                generatedAnalysis={generatedAnalysis}
                setGeneratedAnalysis={setGeneratedAnalysis}
                extractedQAAssessment={extractedQAAssessment}
                setQAAssessment={setQAAssessment}
                shuffledChoicesAssessment={shuffledChoicesAssessment}
                setShuffledChoicesAssessment={setShuffledChoicesAssessment}
                ...
                isStartAssessmentButtonStarted={isStartAssessmentButtonStarted}
            ...
            {showAssessmentPage && (
              <AssessmentPage
                ...
                successfullyInviting={successfullyInviting}
              />
            )}
          </div>
                  ...
                  <div
                    className={`${
                      ...
                      !extraLargeDevices && !largeDevices
                        ...
                        : ""
                    } mbg-100 shadows rounded h-[100vh]`}
                    style={{ zIndex: 50 }}
                  >
                    <div
                      className={`${
                        (extraLargeDevices || largeDevices) && "hidden"
                      } w-full flex items-center justify-end pr-4 py-3`}
                      style={{ zIndex: 60 }}
                    >
                      <button onClick={toggleChatInfo}>
                    ...
                    <div className="flex justify-center">
                      ...
                      >
                        <div>
                          ...
                          >
                            <p className="font-medium">
                              ...
                              <span className="font-bold text-emerald-500">
                                {userList[0]?.username === userData.username
                                  ? "You"
                                  : userList[0]?.username}
                              </span>{" "}
                              - host
                            ...
                            >
                              <button
                                className={`mbg-200 px-2 text-sm py-2 rounded border-thin-800 ${
                                  extraSmallDevice || smallDevice
                                    ? `${
                                        smallDevice
                                          ? isRunningReview
                                ...
                                onClick={() => setHideModalReview("")}
                              >
                                Users
                                <span className="ml-1 bg-red mcolor-100 px-2 rounded-full">
                                  {userList.length >= 1000
                                    ? `${(userList.length / 1000).toFixed(0)}k`
                                    : userList.length}
                                </span>
                              </button>
                              {isStartStudyButtonStarted && (
                                <button
                                  className={`bg-red mcolor-100 px-4 py-2 rounded text-sm ${
                                    extraSmallDevice || smallDevice
                                      ? `${smallDevice ? "w-1/2" : "w-full"}`
                                      : ""
                                  }`}
                                  onClick={leaveReviewerPageRoom}
                                >
                                  Leave Room
                                </button>
                              )}
                          ...
                          <div
                            className={`${hideModalReview} absolute top-0 left-0 modal-bg w-full h-full`}
                          >
                              ...
                              <div
                                className={`mbg-100 max-h-[60vh] ${
                                  extraLargeDevices || largeDevices
                                    ...
                                    ? "w-2/3"
                                    : "w-full mx-2"
                                } relative p-10 rounded-[5px] flex items-center justify-center`}
                                style={{ overflowY: "auto" }}
                              >
                                <button
                                  className="absolute right-4 top-3 text-xl"
                                  onClick={() => setHideModalReview("hidden")}
                                >
                                  ✖
                                </button>
                        ...
                        >
                          ...
                          <div
                            className={`chat-body pb-20 h-full`}
                            style={{ overflowY: "auto" }}
                          >
                        ...
                        <div>
                          <div>
                            ...
                            userList[userTurn]?.userId === userId ? (
                              <div>
                                <p
                                  className={`px-5 py-3 rounded-[5px] text-center ${
                                    extraLargeDevices || largeDevices
                                      ? "text-xl"
                                      : extraSmallDevice
                                      ? "text-md"
                                    ...
                                    userList.length > 0 &&
                                    userList[userTurn]?.userId === userId
                                      ? "font-bold mcolor-100 mbg-800-opacity"
                                      : "font-bold mbg-200 mcolor-800"
                                  }`}
                                >
                                  {userList[userTurn]?.userId !== userId
                                    ? `${
                                        userList[userTurn]?.username
                                          .charAt(0)
                                          .toUpperCase() +
                                        userList[userTurn]?.username.slice(1)
                                      }'s turn`
                                    : "YOUR TURN"}
                                </p>
                                ...
                                {isRunningReview === true && (
                                  ...
                                  >
                                    ...
                                    <h1 className="mcolor-900 text-sm pt-1">
                                      Remaining time: {secondsReview} seconds
                                    </h1>
                                  </div>
                                )}
                                ...
                                {extractedQA && extractedQA.length > 0 ? (
                                  <div>
                                    {/* question */}
                                    <div
                                      className={`flex items-center justify-between gap-4 mt-4 pb-8 text-center font-medium  ${
                                        extraLargeDevices || largeDevices
                                          ? "text-xl"
                                    ...
                                    >
                                      <div
                                        className={`w-full mbg-300 mcolor-900 ${
                                          extraSmallDevice || smallDevice
                                            ? "px-5"
                                            : "px-10"
                                        } pt-5 pb-8 w-full rounded-[5px] mcolor-800`}
                                      >
                                        <p
                                          className={`mcolor-800 ${
                                            extraLargeDevices || largeDevices
                                              ? "text-md"
                                              : extraSmallDevice
                                              ? "text-xs"
                                        ...
                                        {(extractedQA[questionIndex]
                                          ...
                                          extractedQA[questionIndex]
                                            .quizType === "FITB") && (
                                          <div>
                                            <p className="mcolor-800 text-lg mt-2 font-medium  top-10 left-5">
                                              Remaining Hints: {remainingHints}
                                            </p>
                                            <button
                                              className="mcolor-800 mbg-200 border-thin-800 rounded-[5px] px-2 py-1 text-lg mt-2 font-medium  bottom-5 left-5"
                                              onClick={giveHint}
                                            >
                                              Use hint
                                            </button>
                                        ...
                                        {/* questions */}
                                        {extractedQA[questionIndex].quizType ===
                                        "ToF" ? (
                                          <p
                                            className={` ${
                                              extraSmallDevice
                                        ...
                                        <div className="flex justify-center">
                                          <div
                                            className={`dragHere ${
                                              extraSmallDevice || smallDevice
                                                ? "w-full px-5 h-[8vh] text-sm"
                                                : "w-1/2 px-10 h-[12vh] text-md"
                                            } rounded-[5px] bottom-14 flex justify-center items-center mbg-100 ${borderMedium}`}
                                            onDrop={handleDrop}
                                            ...
                                            <div className="text-center">
                                              ...
                                              {(extractedQA[questionIndex]
                                                ...
                                                <input
                                                  ...
                                                  className="w-full h-full text-center py-5 my-2"
                                  ...
                                  <p className="text-center my-5 text-xl mcolor-500">
                                    Nothing to show
                                  </p>
                                )}
                                  ...
                                  extractedQA[questionIndex] && (
                                    <div>
                                          ...
                                          <div
                                            className={`text-red  ${
                                              extraLargeDevices || largeDevices
                                                ...
                                                ? "text-sm"
                                                : "text-xs"
                                            } quicksand font-bold text-center mb-3`}
                                          >
                                            {selectedChoice === ""
                                              ? "No answer."
                                              : extractedQA[questionIndex]
                                                  .quizType !== "MCQA" &&
                                                userList[userTurn]?.points > 0
                                              ? "Wrong. You lost 3 points"
                                              : "Wrong answer."}
                                          </div>
                                        )}
                                      ...
                                      {failCount < 2 &&
                                        ...
                                        gainedPoints === false && (
                                          <p
                                            className={`pb-5 pt-2 text-center font-normal  ${
                                              extraLargeDevices || largeDevices
                                                ? "text-lg"
                                                : mediumDevices || smallDevice
                                                ? "text-sm"
                                        ...
                                        (extractedQA[questionIndex].quizType ===
                                          ...
                                          <div>
                                            ...
                                            {failCount < 2 &&
                                              ...
                                              gainedPoints === false && (
                                                <p
                                                  className={`pb-5 pt-4 text-center font-normal  ${
                                                    extraLargeDevices ||
                                                    largeDevices
                                                      ? "text-lg"
                                                      : mediumDevices ||
                                      ...
                                      {extractedQA[questionIndex].quizType ===
                                        ...
                                        >
                                          {shuffledChoices &&
                                            shuffledChoices[questionIndex].map(
                                              (choice, index) => {
                                                return (
                                                  <div
                                                    key={index}
                                      ...
                                      {disableSubmitButton === false && (
                                        <div className="flex justify-center my-8">
                                          <button
                                            className={`w-1/2 py-2 px-5 mbg-800 rounded-[5px] mcolor-100  ${
                                              extraLargeDevices || largeDevices
                                                ? "text-lg"
                                                : mediumDevices || smallDevice
                                                ? "text-sm"
                              ...
                              <div>
                                <p
                                  className={`mbg-200 mcolor-800 px-5 py-3 rounded-[5px] text-center text-xl ${
                                    userList.length > 0 &&
                                    userList[userTurn]?.userId === userId
                                      ? "font-bold"
                                      : "font-bold"
                                  ...
                                  <div
                                    className={`${
                                      extraSmallDevice
                                        ? "text-center text-xs px-5"
                                        : "text-sm px-10"
                                    } timer-container mbg-input pt-3 pb-1`}
                                    style={{ zIndex: 55 }}
                                  >
                                    <div
                                      className="rounded-[5px]"
                                ...
                                {extractedQA && extractedQA.length > 0 ? (
                                  <div>
                                    ...
                                    >
                                      <div
                                        className={`w-full mbg-300 mcolor-900 px-10 pt-5 pb-8 w-full rounded-[5px] mcolor-800`}
                                      >
                                        <p
                                          className={`mcolor-800 ${
                                            extraLargeDevices || largeDevices
                                          ...
                                          <div
                                            className={`dragHere ${
                                              extraSmallDevice || smallDevice
                                                ? "w-full px-5 h-[8vh] text-sm"
                                                : "w-1/2 px-10 h-[12vh] text-md"
                                            } rounded-[5px] flex justify-center items-center px-10 mbg-100 ${borderMedium}`}
                                            onDrop={handleDrop}
                                            onDragOver={handleDragOver}
                                          >
                                            {/* answer */}
                                    ...
                                    {(failCount === 2 ||
                                      ...
                                      lostPoints === true && (
                                        ...
                                        >
                                          You have gained 5 points as a result
                                          of {userList[userTurn]?.username}'s
                                          incorrect answer.
                                        </div>
                                      )}
                                    ...
                                    {gainedPoints === true && (
                                      ...
                                      >
                                        {userList[userTurn]?.username} earned 5
                                        points
                                      </div>
                                    )}
                                    ...
                                    {failCount <= 2 &&
                                      ...
                                      gainedPoints === false && (
                                        <p
                                          className={`pb-5 pt-4 text-center font-normal text-lg ${
                                            selectedChoice ===
                                            extractedQA[questionIndex].answer
                                              ? "text-emerald-500"
                                              : "text-red"
                                          }`}
                                        >
                                      ...
                                      gainedPoints === false && (
                                        ...
                                        >
                                          {userList[userTurn]?.userId !== userId
                                            ...
                                            : "You"}{" "}
                                          submitted a wrong answer. {failCount}{" "}
                                          chance left
                                        </p>
                                      )}
                                                ...
                                                <li
                                                  className={`${
                                                    ...
                                                    largeDevices
                                                      ...
                                                      ? "text-sm"
                                                      : "text-xs"
                                                  } quicksand font-bold text-center choice border-thin-800 rounded-[5px]`}
                                                  key={index}
                                                >
                                                  {choice}
                                                </li>
                                  ...
                                  <p
                                    className={`text-center my-5  ${
                                      extraLargeDevices || largeDevices
                                        ...
                                        ? "text-sm"
                                        : "text-xs"
                                    } quicksand font-bold mcolor-500`}
                                  >
                                    Nothing to show
                                  </p>
                                )}
                          ...
                          {itemsLength === itemsDone && (
                            ...
                            >
                              <div className="flex items-center justify-center h-full w-full">
                                <div
                                  className={`relative mbg-100 min-h-[75vh] ${
                                    extraLargeDevices || largeDevices
                                      ? "w-1/2"
                                      : mediumDevices || smallDevice
                                      ? "w-2/3"
                                ...
                                >
                                  ...
                                  <div
                                    ...
                                    } items-center justify-between mb-5 gap-5`}
                                  >
                                    <button
                                      onClick={resetAndStudy}
                                      className={`${
                                        extraSmallDevice ? "w-full" : "w-1/2"
                                      } py-2 rounded mcolor-900 mbg-200 border-thin-800`}
                                    >
                                      Reset Points & Study Again
                                    </button>
                                    <button
                                      onClick={studyAgain}
                                      className={`${
                                        extraSmallDevice ? "w-full" : "w-1/2"
                                      } py-2 rounded mcolor-100 mbg-700`}
                                    >
                                      Study Again
                                    </button>
                                  </div>
                                    ...
                                    {[...userList]
                                      ...
                                      .map((user, index) => {
                                        ...
                                        if (user.points === userScores[0]) {
                                          ...
                                          user.points === userScores[1]
                                        ) {
                                          medalClass = "silver-medal";
                                        } else if (
                                          user.points === userScores[2]
                                        ) {
                                        ...
                                        return (
                                          ...
                                          >
                                            <p
                                              className={`mbg-200 shadows p-3 flex justify-between items-center font-medium rounded mcolor-800`}
                                            >
                                              <span>
